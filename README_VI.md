...
| Tham số | Mô tả |
|--------|--------|
| **Grid distance (pips)** | Khoảng cách giữa hai bậc lưới liền kề. Mặc định: 2000. |
| **Max grid levels per side** | Số bậc tối đa phía trên và phía dưới đường cơ sở. Mặc định: 30. |
...
- **Enable Balance** – Dùng pool để cắt lệnh AA lỗ khi (pool + lỗ) ≥ **20 USD** (cố định) và ≥ 0; cooldown **300 giây** (cố định) sau khi cắt. Cân bằng chỉ chạy khi giá cách đường gốc ít nhất **5 bậc** lưới.
...
- Ngưỡng **20 USD** và cooldown **300 s** cố định (không có input) cho AA, BB, CC. Pool chung; **giá hiện tại** phải cách đường gốc ít nhất **5 bậc** lưới; cooldown sau khi đóng.
...
- Cấu trúc giống AA: Enable, lot, Fixed/Geometric, hệ số nhân, max lot, Take profit, Enable Balance (20 USD, 300 s cooldown). Cân bằng dùng logic thống nhất (xa nhất trước, cùng bậc: AA → BB → CC).

### 2.4 CC (cài đặt)

- Logic giống BB, tham số riêng: Enable, lot, Fixed/Geometric, hệ số nhân, max lot, Take profit, Enable Balance (20 USD, 300 s cooldown). Cân bằng dùng logic thống nhất (xa nhất trước, cùng bậc: AA → BB → CC).
...
- **Start trailing when open profit ≥ (USD)** – Ngưỡng để vào chế độ gồng lãi. Mặc định: 50.
- **Khi lãi giảm: Lock profit | Return to initial** – **Lock profit:** khi lãi giảm X% so với đỉnh thì đóng hết và reset (phiên mới). **Return to initial:** khi lãi giảm X% thì thoát gồng lãi và trở về trạng thái ban đầu; chỉ áp dụng nếu SL chưa được đặt tại Point A.
- **% (cả hai chế độ)** – Kích hoạt khi lãi đang mở giảm đúng % này (vd. ngưỡng 100 USD, 20% → kích hoạt ở 80 USD). Mặc định: 20.
- **Point A (pips)** – Base = bậc lưới gần giá hiện tại **nhất tại thời điểm đạt ngưỡng gồng lãi** (cố định cho phiên gồng lãi đó). Buy: Point A = base + số pips này; Sell: Point A = base − số pips này. SL chỉ được đặt tại Point A khi giá chạm base + (Point A pips) + một bước; sau đó SL gồng theo bước. Mặc định: 1500.
- **Trailing step (pips)** – Mỗi bước di chuyển giá thì SL gồng một bước. Mặc định: 1000.

Chỉ lệnh mở trong **phiên hiện tại** mới dùng cho gồng lãi. Thông báo gửi khi reset.

**Reset breakeven (tùy chọn):** Nếu bật, khi bậc mở xa nhất cách gốc ít nhất X bậc và (P/L đang mở + TP đã đóng trong phiên trừ lock) ≥ 0, EA có thể reset (phiên mới). Chỉ tính phiên hiện tại.

**Biểu đồ:** Không vẽ đường (đường gốc, thời điểm phiên, realtime, Point A đều không hiển thị).
...
2.07 – Advanced Grid Trading EA (Pro). AA, BB, CC; đặt lệnh từ bậc gần gốc ra xa; cân bằng **20 USD / 300 s** cố định; giá ≥ **5 bậc** từ gốc; pool = **TP trong phiên trừ lock**; **tiền lock cộng dồn, không reset**; gồng lãi: **Point A** (base tại ngưỡng, SL tại Point A khi giá chạm base + Point A pips + bước, rồi gồng theo bước); **Lock profit | Return to initial** khi lãi giảm X%; không vẽ đường trên biểu đồ; Telegram; Initial balance at EA startup không reset.
